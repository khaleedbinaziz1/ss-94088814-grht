...
export interface ProductSection4Props {
  allProductsRef: React.RefObject<HTMLDivElement | null>;
}

function ProductSection4Inner({ allProductsRef }: ProductSection4Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const productUrl = searchQuery
          ? `${apiBaseUrl}products?q=${encodeURIComponent(searchQuery)}${user?._id ? `&userId=${user._id}` : ''}`
    ...
    fetchData();
  }, [searchQuery, apiBaseUrl, getPriceRange, user?._id]);
  ...
  const renderProductCard = (product: Product) => (
    <motion.div
      key={product._id}
      className="group relative bg-white border-2 border-gray-100 overflow-hidden hover:border-blue-200 transition-all duration-300 rounded-xl shadow-lg hover:shadow-2xl transform hover:-translate-y-1"
    ...
    >
      <div
        ...
        className="relative aspect-square w-full overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 cursor-pointer"
      >
        <Image
      ...
      <div className="p-3 md:p-4">
        ...
        {product.brand && (
          <p className="text-xs text-gray-500 mb-2 font-medium bg-gray-100 px-2 py-1 rounded-full inline-block">{product.brand}</p>
        )}
        ...
        <div className="flex gap-1.5 md:gap-2">
          <button
            ...
            className="flex-1 bg-primary hover:bg-primary/90 flex items-center justify-center p-1.5 md:p-2 transition-all duration-300 text-white text-xs md:text-sm font-bold rounded-l-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            title="Add to Cart"
          >
          ...
          <button
            ...
            disabled={product.stockStatus === "OUT OF STOCK"}
            className="flex-1 inline-flex items-center justify-center bg-secondary hover:bg-secondary/90 px-2 py-1.5 md:py-2 text-xs md:text-sm font-bold text-white transition-all duration-300 rounded-r-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
            BUY
          </button>
        </div>
...
const ProductSection4: React.FC<ProductSection4Props> = (props) => (
  <Suspense fallback={<div>Loading...</div>}>
    <ProductSection4Inner {...props} />
  </Suspense>
);
