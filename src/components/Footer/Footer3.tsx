...
const Footer3 = () => {
  ...
  const SocialIcon = useCallback(({ name, url }: { name: string; url: string }) => (
    <a
      ...
      className="group relative flex items-center justify-center w-12 h-12 bg-white/10 backdrop-blur-sm rounded-full hover:bg-secondary/20 transition-all duration-300 hover:scale-110 border border-white/20 hover:border-accent/50"
    >
  ...
  return (
    <footer className="bg-secondary text-white relative overflow-hidden p-10">
      {/* Removed gradient overlays for solid secondary background */}
          ...
          <div className="space-y-4">
            {contactInfo.map(({ key, value, icon }) => (
              <div key={key} className="flex items-start space-x-3 text-sm group">
                <div className="flex-shrink-0 w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-xs group-hover:bg-secondary/20 transition-colors duration-300">
                  {icon}
                </div>
